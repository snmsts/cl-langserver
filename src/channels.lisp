...
(defmacro channels () `(connection-channels *client-connection*))
(defmacro channel-counter () `(connection-channel-counter *client-connection*))
...
(defmethod initialize-instance :after ((ch channel) &key)
  ...
  (with-slots (thread) ch
    (when (use-threads-p)
      (setf thread (spawn-channel-thread *client-connection* ch)))
...
(defun send-to-remote-channel (channel-id msg)
  (send-to-client `(:channel-send ,channel-id ,msg)))
