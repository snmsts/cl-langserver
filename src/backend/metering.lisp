...
;;;     (ls-monitor:with-monitoring (names*) ()
;;;         your-forms*)
;;; or
;;;     (ls-monitor:monitor-form your-form)
...
;;; not evaluated. Calls the function LS-MONITOR::MONITORING-ENCAPSULATE on each
...
(defpackage "LS-MONITOR" (:use "COMMON-LISP")
        ...
        (if (> num-no-calls 20)
            (format *trace-output*
                    "~%~@(~r~) monitored functions were not called. ~
                      ~%See the variable ls-monitor::*no-calls* for a list."
                    num-no-calls)
            (format *trace-output*
